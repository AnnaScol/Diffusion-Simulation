...
[sphere_X,sphere_Y,sphere_Z,cutout_disk,cutout_idx_] = CutOutSphere(MainSphereRadius,5,50,N);
...
sphere_X(cutout_idx) = nan;
sphere_Y(cutout_idx) = nan;
sphere_Z(cutout_idx) = nan;
...
%     d_particle = ((x-sphere_X).^2+(y-sphere_Y).^2+(z-sphere_Z).^2)./100; %// compute squared distances
%     no_travel = (sphere_X).^2+(sphere_Y).^2+(sphere_Z).^2; %// compute squared distances

%     if max(d,[],'omitnan') > 10^2
%         disp("beep")
%     end
%     result = d_particle.*squeeze(cutout_disk); 
    ...
    for num_cutouts = 1:size(cutout_disk,1)
        for i = length(sphere_X)
            if ( (sphere_X(i) <= x) && (sphere_Y(i) <= y) && (sphere_Z(i) <= z) )
                result = 1;
    %             disp("beep");
                break
            else
