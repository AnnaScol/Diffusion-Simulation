...
rand_x_steps = 0.02*(rand((STOP_TIME-START_TIME)*movements_per_sec)-1);
rand_y_steps = 0.02*(rand((STOP_TIME-START_TIME)*movements_per_sec)-1);
...
for step = 2 : numberOfSteps
	...
	particle_location(step, 1) = particle_location(step, 1) + rand_x_steps(step);	
	particle_location(step, 2) = particle_location(step, 2) + rand_y_steps(step);
